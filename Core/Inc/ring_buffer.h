...
typedef struct {
	uint8_t *buffer; //Pointer to memory location, the '*' is to indicate a memory adress
	uint16_t head; //Head of ring buffer is defined, to write new data
	uint16_t tail; //Tail of ring buffer is defined, to read old data

	uint16_t capacity; //Buffer capacity
	uint8_t is_full; //Indicates when buffer is full
